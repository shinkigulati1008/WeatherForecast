...
const CloudsAnimation = ({day}) => {
  const mount = useRef(null);

  useEffect(() => {
    // Scene
    const scene = new THREE.Scene();
    //scene.background = new THREE.Color(0x87ceeb); // Set sky blue background color

    // Camera
    const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    camera.position.set(0, 0, 1.5); // Update camera position
...
const fragmentShader = /* glsl */`
    ...
    const animate = () => {
      requestAnimationFrame(animate);
        ...
        mesh2.rotation.x = - performance.now() / 7500;
